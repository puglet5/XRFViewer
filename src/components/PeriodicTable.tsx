...
export default function PeriodicTable({ visible, updateSelectedElements, selectedElements }: Props) {
    ...
    if (tableRef.current) {
      ...
      if (elementId) {
        ...
        if (selectedElements.includes(elementNumber)) {
          ...
          updateSelectedElements([...selectedElements, elementNumber].sort((a, b) => a - b))
        }
      }
    } else { return }
  }
  ...
  return (
    <div className={`${visible ? "block" : "hidden"} fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%)] max-h-full bg-gray-500 bg-opacity-60`} >
